...
(function () {
  ...
  if (!document.getElementById('sp-mobile-nav-css')) {
    var link = document.createElement('link'); link.id='sp-mobile-nav-css'; link.rel='stylesheet'; link.href='/mobile-nav-fix.css?v=20260904-nav5'; document.head.appendChild(link);
  }
  ...
  function boot(){bindMenu();try{new MutationObserver(function(){if(window.innerWidth<1200&&!document.getElementById('hamburger')){var h=document.querySelector('.hamburger');if(h&&h.dataset)h.dataset.spMenuReady='';bindMenu()}}).observe(document.documentElement,{childList:true,subtree:true})}catch(err){}}
...
(function(){
  ...
  function addSchema(){
    ...
    if(/^\/(admin|portal|jarvis)(\/|$)/i.test(path))return;
    var canonical=location.origin+(path==='/'?'/':path.replace(/\/index\.html$/,'/'));
    var graph=[
      {"@type":"Organization","@id":location.origin+'/#organization',"name":"Stagepulse","url":location.origin+'/',"logo":location.origin+'/favicon.svg',"telephone":"+90-532-068-3012","email":"teklifal@stagepulse.com.tr","areaServed":{"@type":"Country","name":"Türkiye"},"sameAs":["https://www.instagram.com/stagepulse.hatay"]},
      {"@type":"WebSite","@id":location.origin+'/#website',"url":location.origin+'/',"name":"Stagepulse","inLanguage":document.documentElement.lang||'tr-TR',"publisher":{"@id":location.origin+'/#organization'}},
      {"@type":"WebPage","@id":canonical+'#webpage',"url":canonical,"name":document.title,"inLanguage":document.documentElement.lang||'tr-TR'}
    ];
    var s=document.createElement('script');s.type='application/ld+json';s.dataset.stagepulseSeoSchema='1';s.textContent=JSON.stringify({"@context":"https://schema.org","@graph":graph});document.head.appendChild(s);
  }
  function run(){
    var path=location.pathname.replace(/\/+/g,'/');
    if(/^\/(admin|portal|jarvis)(\/|$)/i.test(path))return;
    if(!document.querySelector('meta[name="robots"]')){var r=document.createElement('meta');r.name='robots';r.content='index, follow';document.head.appendChild(r)}
    if(!document.querySelector('link[rel="canonical"]')){var c=document.createElement('link');c.rel='canonical';c.href=location.origin+(path==='/'?'/':path.replace(/\/index\.html$/,'/'));document.head.appendChild(c)}
    addSchema();
  }
