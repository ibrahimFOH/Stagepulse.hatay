...
async function requireAdmin(req:Request){const h=req.headers.get('authorization')||'';if(!h.toLowerCase().startsWith('bearer '))throw new Error('UNAUTHENTICATED');const {data,error}=await admin.auth.getUser(h.slice(7).trim());if(error||!data.user)throw new Error('UNAUTHENTICATED');const {data:p,error:pe}=await admin.from('admin_profiles').select('active').eq('user_id',data.user.id).maybeSingle();if(pe||!p?.active)throw new Error('FORBIDDEN');return data.user;}
