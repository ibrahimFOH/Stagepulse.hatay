...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(b.dispatch_token){
      ...
      ids=Array.isArray(b.user_ids)?b.user_ids.filter((x:unknown):x is string=>typeof x==='string'):[];
    ...
    return out(req,{error:m,statusCode:(e as any)?.statusCode||0,providerBody:(e as any)?.body||null},s);
  }
});
