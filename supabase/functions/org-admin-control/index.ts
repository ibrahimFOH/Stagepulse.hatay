  ...
  try{
    ...
    if(action==="create_member"){
      ...
      try{
        ...
        if(caps.length){const {data:validCaps,error:ve}=await db.from("admin_capabilities").select("key").in("key",caps).eq("active",true);if(ve)throw ve;const allowed=new Set((validCaps||[]).map((x:any)=>x.key));const rows=caps.filter((k:string)=>allowed.has(k)).map((key:string)=>({user_id:created.user.id,capability_key:key,enabled:true,granted_by:actor,updated_at:iso()}));if(rows.length){const {error:ge}=await db.from("admin_capability_grants").upsert(rows,{onConflict:"user_id,capability_key"});if(ge)throw ge;}}
        await audit(actor,"org_member_created","org_membership",created.user.id,{username,roleCode});return out(req,{ok:true,user_id:created.user.id});
