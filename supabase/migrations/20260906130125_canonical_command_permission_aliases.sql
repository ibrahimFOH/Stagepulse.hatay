...
-- Preserve existing capability names while routing all management writes through the canonical command surface.
...
as $function$
declare v_user uuid:=auth.uid(); v_table text; v_perm text; v_alias text; v_result jsonb; v_set text; v_where text; v_payload jsonb:=coalesce(p_payload,'{}'::jsonb);
begin
 if v_user is null then raise exception 'AUTH_REQUIRED'; end if;
 if not private.is_org_owner() then
  v_alias:=case p_entity when 'services' then 'pricing' when 'payments' then 'finance' else p_entity end;
  v_perm:=case p_action when 'create' then v_alias||'.create' when 'update' then v_alias||'.update' when 'delete' then v_alias||'.delete' when 'approve' then case when p_entity='offers' then 'offers.manage' else 'approvals.manage' end when 'link' then 'resources.manage' else v_alias||'.manage' end;
  if not private.staff_has_exact_perm(v_perm) and not private.staff_has_exact_perm(v_alias||'.manage') then raise exception 'PERMISSION_DENIED'; end if;
 end if;
