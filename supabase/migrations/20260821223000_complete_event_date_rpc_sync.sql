...
BEGIN
  ...
  IF old_date IS NULL THEN delta := interval '0'; ELSE delta := make_interval(days => (p_event_date - old_date)); END IF;
  ...
  FOR j IN SELECT * FROM public.jobs WHERE offer_id = p_offer_id FOR UPDATE LOOP
    ...
    SELECT s.user_id,'event_change','Etkinlik tarihi değişti','Etkinlik tarihi güncellendi.',p_offer_id
  ...
  VALUES(u,'event_date_changed','teklifler',p_offer_id,jsonb_build_object('old_date',old_date,'new_date',p_event_date,'jobs_updated',job_count));
