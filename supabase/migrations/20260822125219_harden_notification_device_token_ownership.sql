-- Prevent an authenticated user from reassigning another user's FCM token.
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.notification_devices d WHERE d.token=trim(p_token) AND d.user_id<>auth.uid()) THEN
  ...
  ON CONFLICT(token) DO UPDATE SET platform=excluded.platform,app_variant=excluded.app_variant,active=true,last_seen_at=now(),updated_at=now()
  RETURNING * INTO r;
  RETURN r;
END;
$$;
