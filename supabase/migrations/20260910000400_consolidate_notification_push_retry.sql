...
begin
  ...
  set push_dispatch_token = gen_random_uuid(),
      ...
      push_next_attempt_at = now() + make_interval(secs => least(900, (30 * power(2, n.push_attempt_count))::integer)),
  ...
  begin
    select net.http_post(
      ...
      body := jsonb_build_object('notification_id', p_notification_id, 'dispatch_token', v_token)
    ) into v_request_id;
  exception when others then
    update public.notifications
    set push_last_error = left('pg_net enqueue failed: ' || sqlerrm, 2000),
        push_next_attempt_at = now() + interval '1 minute'
    where id = p_notification_id and push_dispatch_token = v_token and push_dispatched_at is null;
    return null;
  end;
  return v_request_id;
end;
...
set search_path = 'public, extensions, pg_temp'
as $$
declare v_id bigint; v_queued integer := 0;
begin
  for v_id in
    select n.id from public.notifications n
    where n.push_dispatched_at is null and n.push_attempt_count < 6 and n.push_next_attempt_at <= now()
      and (n.push_claimed_at is null or n.push_claimed_at < now() - interval '5 minutes')
    order by n.push_next_attempt_at, n.id for update skip locked limit 50
  loop
    if private.enqueue_notification_push(v_id) is not null then v_queued := v_queued + 1; end if;
  end loop;
  return v_queued;
end;
$$;
...
create trigger trg_notification_push_dispatch after insert on public.notifications
...
begin
  ...
  select recipients.user_id, 'new_quote', 'Yeni teklif talebi', left(v_body, 4000), new.id
  from (
    select membership.user_id from public.org_memberships membership
...
create trigger trg_new_offer_notification after insert on public.teklifler for each row execute function public.on_quote_after_notify();

do $$
begin
  if exists (select 1 from cron.job where jobname = 'stagepulse-notification-push-retry') then
    perform cron.unschedule('stagepulse-notification-push-retry');
  end if;
  perform cron.schedule('stagepulse-notification-push-retry', '* * * * *', 'select private.retry_notification_pushes();');
end;
$$;

comment on function public.dispatch_notification_push() is 'PROTECTED CORE: queues bounded notification delivery attempts through pg_net; delivery is acknowledged only by send-fcm-notification.';
