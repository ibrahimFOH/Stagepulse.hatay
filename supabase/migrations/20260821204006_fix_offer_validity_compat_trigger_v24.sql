...
BEGIN
  ...
  ELSE
    IF NEW.valid_until IS DISTINCT FROM OLD.valid_until THEN
      NEW.validity_until := CASE WHEN NEW.valid_until IS NULL THEN NULL ELSE NEW.valid_until::timestamp AT TIME ZONE 'Europe/Istanbul' END;
    ELSIF NEW.validity_until IS DISTINCT FROM OLD.validity_until THEN
      NEW.valid_until := CASE WHEN NEW.validity_until IS NULL THEN NULL ELSE (NEW.validity_until AT TIME ZONE 'Europe/Istanbul')::date END;
    END IF;
  END IF;
  RETURN NEW;
END;
