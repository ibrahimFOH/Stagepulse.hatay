...
internal object AndroidUrlPolicy {
    ...
    fun isTrustedReleaseUrl(value: String): Boolean {
        return parseHttps(value)?.let { uri ->
            ...
            parts.size == 7 && parts[1] == "ibrahimFOH" && parts[2] == "Stagepulse.hatay" && parts[3] == "releases" && parts[4] == "download" && parts[5].isNotBlank() && parts[6].isNotBlank() && parts[6].endsWith(".apk", ignoreCase = true)
        } ?: false
    }
    ...
    fun isTrustedApkDownloadHop(value: String): Boolean {
        ...
        return parseHttps(value)?.let { uri ->
            ...
            val path = uri.rawPath ?: return false
            val parts = safePathSegments(path) ?: return false
            path.startsWith("/github-production-release-asset/") && !path.contains('%') && parts.size >= 3 && parts.drop(1).all { it.isNotBlank() } && isSafeQuery(uri.rawQuery)
        } ?: false
    }

    fun isAllowedApkRedirect(from: String, to: String): Boolean = isTrustedApkDownloadHop(from) && isTrustedApkDownloadHop(to)
    ...
    private fun parseHttps(value: String): URI? {
        return try { val uri = URI(value); if (!uri.scheme.equals("https", ignoreCase = true) || uri.host.isNullOrBlank() || uri.userInfo != null || uri.port != -1) null else uri } catch (_: Exception) { null }
    }
    ...
    private fun safePathSegments(rawPath: String): List<String>? {
        ...
        return rawPath.split('/').map { raw ->
            var decoded = raw
            repeat(8) {
                val next = try { URLDecoder.decode(decoded, StandardCharsets.UTF_8.name()) } catch (_: Exception) { return null }
                if (next == decoded) return@repeat
                decoded = next
            }
            if (decoded == "." || decoded == ".." || decoded.contains('%') || decoded.contains('/') || decoded.contains('\\') || decoded.contains('\u0000')) return null
            decoded
        }
    }
    ...
    private fun isSafeQuery(rawQuery: String?): Boolean {
        ...
        repeat(3) { decoded = try { URLDecoder.decode(decoded, StandardCharsets.UTF_8.name()) } catch (_: Exception) { return false } }
