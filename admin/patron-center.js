...
(() => {
  ...
  let state=null, selected=null, owner=false, loading=false;
  ...
  async function load(){
    ...
    try{
      ...
      state=r.data;
      const nonOwner=(state.admin_members||[]).filter(x=>x.role_code!=='owner');
      selected=selected&&nonOwner.some(x=>x.user_id===selected)?selected:(nonOwner[0]?.user_id||null);
      render();
  ...
  async function open(){
    if(location.hash.slice(1)!=='patron-center')return;
    if(!owner)owner=await isOwner();
    if(!owner){renderDenied();return}
    load().catch(e=>{console.error(e);const root=$('#content');if(root)root.innerHTML=`<div class="panel"><h2>Patron Merkezi yüklenemedi</h2><p class="muted">${esc(e.message||'Bilinmeyen hata')}</p></div>`});
  }

  window.addEventListener('stagepulse-admin-ready',open);
  window.addEventListener('stagepulse:admin-ready',open);
  window.addEventListener('stagepulse:logged-in',open);
