(() => {
  ...
  function initOfferExperience() {
    ...
    sections.forEach((section,index)=>{const card=document.createElement('section');card.className=`sp-offer-section ${index===0?'is-open':''}`;card.dataset.spSection=section.key;const head=document.createElement('button');head.type='button';head.className='sp-section-head';head.innerHTML=`<span class="sp-section-number">${section.number}</span><span class="sp-section-copy"><small>${section.eyebrow}</small><strong>${section.title}</strong><em>${section.text}</em></span><span class="sp-section-toggle"><i class="fa-solid fa-chevron-down"></i></span>`;const body=document.createElement('div');body.className='sp-section-body';section.ids.forEach(id=>{const group=groupFor(id);if(group)body.appendChild(group)});card.appendChild(head);card.appendChild(body);cards.appendChild(card);head.addEventListener('click',()=>{const open=card.classList.contains('is-open');$$('.sp-offer-section',cards).forEach(x=>x.classList.remove('is-open'));if(!open)card.classList.add('is-open');updateProgress(section.key);});});
    ...
    if(submit){submit.classList.add('sp-final-submit');submit.type='button';submit.removeAttribute('form');submitRow.appendChild(submit)}
    ...
    $$('[data-sp-jump]',progress).forEach(btn=>btn.addEventListener('click',()=>{const key=btn.dataset.spJump;const card=$(`[data-sp-section="${key}"]`,cards);if(!card)return;$$('.sp-offer-section',cards).forEach(x=>x.classList.remove('is-open'));card.classList.add('is-open');card.scrollIntoView({behavior:'smooth',block:'start'});updateProgress(key)}));
