...
(() => {
  ...
  const repair = () => {
    ...
    if(nav.dataset.spNavigationIntegrity===signature){nav.querySelectorAll('button[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===active));return;}
    ...
    for(const [group,views] of GROUPS){
      const matches=desired.filter(item=>views.has(item[0])); if(!matches.length) continue;
      const label=document.createElement('div'); label.className='portal-nav-label'; label.textContent=group; frag.appendChild(label);
      for(const item of matches){let b=existing.get(item[0]);if(!b){b=document.createElement('button');b.type='button';b.dataset.view=item[0];b.textContent=item[1];b.setAttribute('aria-label',item[1]);b.addEventListener('click',e=>{e.preventDefault();window.loadView(item[0]);});}b.hidden=false;b.style.display='';b.setAttribute('aria-hidden','false');b.classList.toggle('active',item[0]===active);frag.appendChild(b);}
    }
  ...
  const boot=()=>{repair();window.setTimeout(repair,150);window.setTimeout(repair,700);window.setTimeout(repair,1500);window.addEventListener('stagepulse:permissions-ready',repair);window.addEventListener('stagepulse:permissions-changed',repair);window.addEventListener('hashchange',repair);const nav=document.querySelector('#sideNav');if(nav){const observer=new MutationObserver(()=>{if(!nav.dataset.spNavigationObserverBusy){nav.dataset.spNavigationObserverBusy='1';queueMicrotask(()=>{nav.dataset.spNavigationObserverBusy='';repair();});}});observer.observe(nav,{childList:true,subtree:true});}};
