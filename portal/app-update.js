...
(() => {
  ...
  async function checkForUpdate() {
    ...
    try {
      ...
      const staffWebVersion=String(info?.staff?.web_version || '');
      const adminWebVersion=String(info?.admin?.web_version || '');
      const release=String(info?.release || '');
      const build=staffWebVersion || adminWebVersion || release;
