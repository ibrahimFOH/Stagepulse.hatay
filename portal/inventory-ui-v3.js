...
(() => {
  ...
  async function modal(id){
    ...
    const total=n(e.quantity);document.getElementById('spPInvModal')?.remove();
    document.body.insertAdjacentHTML('beforeend',`<div class="sp-pinv-modal" id="spPInvModal"><div class="sp-pinv-card"><div class="sp-pinv-head"><div><p class="muted small">ENVANTER DURUMU</p><h2>${esc([e.brand,e.model].filter(Boolean).join(' ')||e.category)}</h2><p class="muted">Toplam stok <strong>${total}</strong> adet · Aktif/Pasif admin tarafından yönetilir</p></div><button class="sp-pinv-close" onclick="window.spPersonnelInventoryClose()">×</button></div><div class="sp-pinv-section"><h3>Durum adetlerini değiştir</h3><div class="sp-pinv-status-grid"><label>Arızalı<input id="spPFault" type="number" min="0" max="${total}" value="${n(e.faulty_quantity)}"></label><label>Bakımda<input id="spPMaint" type="number" min="0" max="${total}" value="${n(e.maintenance_quantity)}"></label><label>Rezerve<input id="spPReserved" type="number" min="0" max="${total}" value="${n(e.reserved_quantity)}"></label><label>Kullanımda<input id="spPUse" type="number" min="0" max="${total}" value="${n(e.in_use_quantity)}"></label></div><div class="sp-pinv-live"><div><span>Toplam stok</span><b id="spPTotal">${total}</b></div><div><span>Arızalı</span><b id="spPFaultLive">${n(e.faulty_quantity)}</b></div><div><span>Sağlam</span><b id="spPHealthy">${healthy(e)}</b></div><div><span>Boşta</span><b id="spPAvailable">${available(e)}</b></div></div></div><div class="sp-pinv-actions"><button class="btn btn-primary" onclick="window.spPersonnelInventorySave('${e.id}')">Kaydet</button><button class="btn" onclick="window.spPersonnelInventoryClose()">İptal</button></div></div></div>`);
  ...
  window.spPersonnelInventoryClose=()=>document.getElementById('spPInvModal')?.remove();
