/* Stagepulse Personnel Portal — detailed analytics. */
(() => {
  'use strict';
  ...
  const safeRows = async (promise, fallback=[]) => { try { const r=await promise; if(r?.error) throw r.error; return r?.data || fallback; } catch(e) { console.warn('[analytics]',e); return fallback; } };
