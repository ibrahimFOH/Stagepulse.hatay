/* Stagepulse incoming offers UI — canonical live permission + evaluation workflow */
(() => {
  ...
  const fmt=d=>{if(!d)return '—';const x=new Date(d);return Number.isNaN(x.getTime())?String(d):x.toLocaleString('tr-TR',{dateStyle:'short',timeStyle:'short'});};
  const toast=(m,ok=true)=>window.toast?window.toast(m,ok):alert(m);
  const permission=async key=>{const {data,error}=await client.rpc('staff_capability',{p_capability:key});if(error)throw error;return data===true;};

  async function render(){
    const [{data:userData,error:userError},{data,error}] = await Promise.all([
      client.auth.getUser(),
      client.from('teklifler').select('*').order('created_at',{ascending:false})
    ]);
    if(userError||error){document.querySelector('#content').innerHTML=`<div class="panel"><b>Teklifler yüklenemedi</b><p class="muted">${esc((userError||error).message)}</p></div>`;return;}
    ...
    const rows=(data||[]).map(o=>{
      ...
      return `<article class="incoming-offer-card" data-offer-id="${esc(o.id)}">
        ...
        <div class="incoming-offer-actions">
          ...
          ${!closed&&!evaluating&&canEvaluate?`<button class="btn btn-primary" data-evaluate="${esc(o.id)}">Değerlendir</button>`:''}
          ...
          ${!canEvaluate&&!evaluating?'<span class="muted">Değerlendirme yetkiniz yok</span>':''}
        </div>
      </article>`;
    }).join('');
    ...
    document.querySelectorAll('[data-evaluate]').forEach(btn=>btn.onclick=async()=>{
      ...
      try{const {error:e}=await client.rpc('offer_claim_for_review',{p_offer_id:btn.dataset.evaluate});if(e)throw e;toast('Teklif değerlendirmeye alındı.');await render();}
      catch(e){toast(e.message,false);btn.disabled=false;btn.textContent='Değerlendir';}
    });
    document.querySelectorAll('[data-accept],[data-reject]').forEach(btn=>btn.onclick=async()=>{
      btn.disabled=true;
      const accepted=btn.hasAttribute('data-accept');
      const note=prompt(accepted?'Kabul notu (opsiyonel):':'Red notu (opsiyonel):','')||null;
      try{const {error:e}=await client.rpc('offer_evaluate',{p_offer_id:accepted?btn.dataset.accept:btn.dataset.reject,p_status:accepted?'accepted':'rejected',p_note:note});if(e)throw e;toast(accepted?'Teklif kabul edildi.':'Teklif reddedildi.');await render();}
